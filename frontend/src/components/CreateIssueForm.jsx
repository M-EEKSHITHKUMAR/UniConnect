...
const CreateIssueForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    } catch (err) { toast.error(err || 'Failed to post issue') }
          ...
          <form onSubmit={handleSubmit} style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button type="button" onClick={() => { setExpanded(false); setTitle(''); setDescription(''); clearImage() }}
                  style={{
                ...
                <button type="submit" disabled={loading} style={{
                  ...
                  border: 'none', borderRadius: '10px', cursor: 'pointer', opacity: loading ? 0.6 : 1
                }}>
                  {loading ? 'Posting...' : <><FiSend size={13} /> Post Issue</>}
                </button>
              </div>
            </div>
          </form>
